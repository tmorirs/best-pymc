...
| 頻度論との併記 | なし | Welch の t 検定・Cohen's d・検定力解析を自動併記 |

## 動作確認済み環境

Python 3.12 / PyMC 6.3.1 / ArviZ 1.3.0 / NumPy 2.4.4 / SciPy 1.17.1 でテスト（15 件）が通ることを確認済み。`uv sync` → `uv run pytest` → `uv build` も同環境で確認済みで、`pip` 経由と `uv` 経由で同じ seed から同じ事後平均・HDI が出ることも確認しています。ArviZ は 0.x と 1.x で API が大きく変わるため、HDI などの数値計算は ArviZ に依存せず自前で実装してある（`az.rhat` / `az.ess` のみ使用。両系統に存在する関数）。
...
[参考] Welch t-test: t = 1.622, p = 0.1098, d = 0.331
      検定力 (α=0.05, 両側): 事後 1-β=0.34; 感度 |d|≥0.602 (80%); 事前 n=145/群 (観測d, 80%)
```

Welch の t 検定では p = 0.11 で「有意差なし」、Cohen's d も 0.33（小〜中程度）にとどまる。検定力も事後 0.34 と低く、この標本サイズで 80% 検出力を得るには |d|≥0.60 が必要で、観測 d を 80% で捉えるなら 1 群 145 例が要る。一方ベイズ推定では平均の差が正である事後確率は 0.99、効果量の事後平均は 0.64。外れ値（IQ 82, 123, 124）に引きずられて t 検定の分散推定が膨らむ一方、t 分布尤度はそれを裾として吸収するためこの差が生じる。ただし ROPE を ±1 に設定すると判定は `undecided` になり、「ゼロではなさそうだが実質的に意味のある差かは断定できない」というのがこのデータから言える正しい結論になる。

> **注（ROPE ±1 の位置づけについて）** `examples/smart_drug.py` の既定 ROPE = [-1, 1] は、t 検定とベイズ推定の結論の食い違いをそのまま見せるためのチュートリアル上の設定であり、IQ の心理測定学的な意味での「臨床的に無視できる差」ではない。参考として、IQ（平均100・SD15）の一般的な心理測定学の知見から見積もると：
>
> - **測定誤差 (SEM)**: 主要な IQ テストの信頼性は概ね r ≈ 0.90〜0.97 で、SEM = SD×√(1−r) ≈ 1.9〜4.7 点。つまり ±1〜2 点はテスト自体の測定誤差に埋もれる範囲。
> - **Reliable Change Index (RCI)**: 臨床現場で「本当に変化した」と言うための基準は、実務上おおむね 5 点前後（90% CI）が使われることが多い。
> - **Cohen's d の目安**: SD=15 として small (d=0.2) ≈ 3 点、medium (d=0.5) ≈ 7.5 点。
> - **分類上の意味**: IQ 分類（境界域・平均域・優秀域など）はおよそ 10 点刻みで、教育・臨床上の意思決定に影響するのはこのオーダーの差。
>
> これらを踏まえると、「臨床的に意味がある」と言うための現実的な ROPE はおおよそ ±3〜5 点、場合によってはそれ以上と考えるほうが妥当である。試しに ±5 で実行すると、結論そのものが逆転する:
>
> ```bash
> uv run best-demo --rope -5 5 --overlaid --plot smart_drug_rope5_overlaid.png
> ```
>
> ```
> ROPE 判定 (ROPE = [-5.000, 5.000])
>     実質的に同等（HDI が ROPE 内に収まる）
>     事後確率が ROPE 内にある割合 = 1.000
> ```
>
> ±1 では `undecided`（データ不足で判断保留）だった判定が、±5 では `equivalent`（実質的に同等）へと明確に変わる。同じ事後分布でも ROPE の取り方ひとつで結論が変わりうることの好例であり、「**ROPE は必ずデータを見る前に、分野知識に基づいて決める**」という上記の注意事項の重要性を裏付けている。
